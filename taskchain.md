...
- **Next objective:** reconcile the accepted canonical-record v1 baseline with an independently permissioned retrieval-to-sanitizer artifact handoff, then complete adversarial conformance and retained exact-head evidence.
...
|---|---|---|---|---|---|
| P0 | Disposition the reproducible security and CLI baseline | QSOBuilder / Architect | — | REVIEW | Current baseline is replayed on a final immutable head with clean install, complete tests, verifier, hidden-control scan, exact-source assertion, minimum permissions, retained evidence, and Architect acceptance. |
...
PR #2 remains an open historical P0 candidate, but later `main` changes mean its earlier exact-head evidence cannot serve as final release evidence for current `main`. P0 stays `REVIEW` until the current accepted composition is replayed and reviewed as one immutable head with retained artifacts and no unresolved findings.

## Separate draft candidates

Open proposals for broader collection, a bounded live checkpoint, experimental genome and spawning behavior, consent-lock repair, and QSIO integration remain separate pull-request candidates. They must not be described as accepted repository capability or used to bypass P0-P3 dependencies.
